...
const LuckyDip = (props) => {
  ...
  useEffect(() => {
    if (isClicked && !open) {
      ...
      for (let i = 0; i < 6; i++) {
        let temp =
          90 - (Math.floor(Math.random() * 5) + 5) * Math.abs(i - randWon + 1);
  ...
  return (
    <div style={{ maxWidth: "800px", position: "relative" }}>
      ...
      <div
        className="track"
        style={{ height: `${isClicked ? "auto" : "150px"}` }}
      >
        <img className="track-image" src={background} />
        {!isClicked && PopupDiv}
        <div
          className={`dog-number dog-number1 ${
            isClicked ? "dog-number1-display" : ""
          }`}
        >
          <div className="race-pos-no race-pos-no-1">1</div>
        </div>
        <div
          className={`dog-number dog-number2 ${
            isClicked ? "dog-number2-display" : ""
          }`}
        >
          <div className="race-pos-no race-pos-no-2">2</div>
        </div>
        <div
          className={`dog-number dog-number3 ${
            isClicked ? "dog-number3-display" : ""
          }`}
        >
          <div className="race-pos-no race-pos-no-3">3</div>
        </div>
        <div
          className={`dog-number dog-number4 ${
            isClicked ? "dog-number4-display" : ""
          }`}
        >
          <div className="race-pos-no race-pos-no-4">4</div>
        </div>
        <div
          className={`dog-number dog-number5 ${
            isClicked ? "dog-number5-display" : ""
          }`}
        >
          <div className="race-pos-no race-pos-no-5">5</div>
        </div>
        <div
          className={`dog-number dog-number6 ${
            isClicked ? "dog-number6-display" : ""
          }`}
        >
          <div className="race-pos-no race-pos-no-6">6</div>
        </div>
        <AnimatedObeject
          className={`rabbit ${!open && isClicked ? "rabbit-display" : ""}`}
        ></AnimatedObeject>
        <AnimatedObeject1
          className={`dog dog1 ${!open && isClicked ? "dog1-display" : ""}`}
        ></AnimatedObeject1>
        <div
          className={`dog dog1 ${open ? "dog1-display" : ""}`}
          style={{ left: `${objects[0]}%` }}
        ></div>
        <AnimatedObeject2
          className={`dog dog2 ${!open && isClicked ? "dog2-display" : ""}`}
        ></AnimatedObeject2>
        <div
          className={`dog dog2 ${open ? "dog2-display" : ""}`}
          style={{ left: `${objects[1]}%` }}
        ></div>
        <AnimatedObeject3
          className={`dog dog3 ${!open && isClicked ? "dog3-display" : ""}`}
        ></AnimatedObeject3>
        <div
          className={`dog dog3 ${open ? "dog3-display" : ""}`}
          style={{ left: `${objects[2]}%` }}
        ></div>
        <AnimatedObeject4
          className={`dog dog4 ${!open && isClicked ? "dog4-display" : ""}`}
        ></AnimatedObeject4>
        <div
          className={`dog dog4 ${open ? "dog4-display" : ""}`}
          style={{ left: `${objects[3]}%` }}
        ></div>
        <AnimatedObeject5
          className={`dog dog5 ${!open && isClicked ? "dog5-display" : ""}`}
        ></AnimatedObeject5>
        <div
          className={`dog dog5 ${open ? "dog5-display" : ""}`}
          style={{ left: `${objects[4]}%` }}
        ></div>
        <AnimatedObeject6
          className={`dog dog6 ${!open && isClicked ? "dog6-display" : ""}`}
        ></AnimatedObeject6>
        <div
          className={`dog dog6 ${open ? "dog6-display" : ""}`}
          style={{ left: `${objects[5]}%` }}
        ></div>
      </div>
      {WonBox}
    </div>
